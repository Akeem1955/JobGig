package com.pioneers.jobgig.services.communicate.session
...
import com.pioneers.jobgig.services.communicate.SignalingCommand
import com.pioneers.jobgig.services.communicate.StreamPeerConnection
...
import com.pioneers.jobgig.services.communicate.audio.AudioHandler
import com.pioneers.jobgig.services.communicate.audio.AudioSwitchHandler
...
class WebRtcSessionManagerImpl(
    ...
    private val localVideoTrack: VideoTrack by lazy {
        println("Local Video Track Was Initialized")
    ...
    override fun onSessionScreenReady() {
        println("But Session Was Ready now what now")
        setupAudio()
        peerConnection.connection.addTrack(localVideoTrack)
        peerConnection.connection.addTrack(localAudioTrack)
        sessionManagerScope.launch {
            println("about to emit localVideo Track ")
            println("Checking if it is null ${localVideoTrack != null}")
            // sending local video track to show local video from start
            _localVideoTrackFlow.emit(localVideoTrack)
            println("Emit finished ")
            println(localVideoTrackFlow.replayCache.toList())
            println(_localVideoTrackFlow.replayCache)
            println(_localVideoTrackFlow.subscriptionCount.value)
            println(localVideoTrack)
